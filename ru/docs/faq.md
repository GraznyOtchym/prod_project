...
## Возвращает ли создание правила 422 из-за невалидного DSL?

Нет. `POST /fraud-rules` и `PUT /fraud-rules/{id}` сохраняют `dslExpression` как строку. Для проверки выражения
используйте `POST /fraud-rules/validate`.

## Можно ли сдать без парсера DSL?

Да. На первом этапе допускается уровень поддержки 0 (Tier 0): `/fraud-rules/validate` возвращает `DSL_UNSUPPORTED_TIER`,
