...
- `metadata` (произвольные дополнительные данные транзакции, если используются).
...
Слово `Tier` встречается в названиях уровней и в кодах ошибок (например, `DSL_UNSUPPORTED_TIER`) — это просто название,
менять его не нужно.
...
- `POST /fraud-rules/validate` возвращает `isValid=false` и `errors` с `code=DSL_UNSUPPORTED_TIER` для любого
  `dslExpression`.
...
- `DSL_PARSE_ERROR` — синтаксическая ошибка (для этого кода `position` и `near` обязательны).
- `DSL_INVALID_FIELD` — неизвестное поле DSL.
- `DSL_INVALID_OPERATOR` — оператор неприменим к типу значения (например, `currency > 'RUB'`).
- `DSL_UNSUPPORTED_TIER` — выражение корректно, но использует возможности DSL выше реализованного уровня (частичная
  реализация).
- `DSL_TOO_COMPLEX` — выражение превышает лимит сложности (см. ниже).

### Лимит сложности (защита автопроверки)

Проверка обязана ограничивать размер «дерева выражения», **если вы реализуете парсинг (уровень 1+)**.

Правило:

- если выражение превышает **100 узлов AST**, `/fraud-rules/validate` обязано вернуть `isValid=false` и
  `DSL_TOO_COMPLEX`;
- ограничения на DSL при создании/обновлении правила применяются через `/fraud-rules/validate` (создание само по себе не
  обязано парсить DSL).

Для уровня 0 допустимо всегда возвращать `DSL_UNSUPPORTED_TIER` без построения AST (это проще и безопаснее на старте).
