...
3. [dsl.md](./dsl.md) — как устроен мини-язык правил (уровни поддержки, лимиты).
...
5. DSL на самом простом уровне (Tier 0):
    - `POST /fraud-rules/validate` возвращает `DSL_UNSUPPORTED_TIER`.
...
1. **Считайте все активные правила.** Активные = `enabled=true`. `ruleResults` должен быть полным и идти в одном и том
   же порядке (сортировка: `priority`, затем `id`).
...
> **Параметры пагинации:** `page` — номер страницы (0-based, по умолчанию 0), `size` — размер страницы (1—100, по
> умолчанию 20)
...
Уровень поддержки 0 (Tier 0, разрешенный старт): допускается всегда возвращать `isValid=false` с `DSL_UNSUPPORTED_TIER`и
получить баллы части A.
...
- Ошибки в `errors[].code` обязаны быть машиночитаемыми и не требовать парсинга текста `message`:
    ...
    - `DSL_UNSUPPORTED_TIER` — выражение синтаксически корректно, но использует возможности DSL, которые участник не
      реализовал (это нормальный сценарий частичной реализации).
    - `DSL_TOO_COMPLEX` — выражение слишком сложное (см. лимит ниже).

**Лимит сложности (обязателен для уровня поддержки 1+ / Tier 1+):**

- Если вы реализуете парсер (уровень 1+ / Tier 1+), DSL выражение не должно превышать **100 узлов AST**.
- Если превышает, `/fraud-rules/validate` обязано вернуть `isValid=false` с `DSL_TOO_COMPLEX`.
- Для уровня поддержки 0 (Tier 0) допустимо отвечать `DSL_UNSUPPORTED_TIER` без построения AST.
...
> поддержки/лимиты). Это подсказка: правила могут храниться в БД, а при применении сервис обязан работать устойчиво (
> см. [dsl.md](./dsl.md)).
...
- `from`, `to` — фильтр по периоду (ISO 8601).
- `page`, `size` — пагинация.
...
|------------------------|------------------|-------------------------------------------------------------------|
| `userId`               | Только для ADMIN | UUID пользователя. Для USER игнорируется и берется из JWT (`sub`) |
| `amount`               | Да               | Сумма (0.01 - 999999999.99)                                       |
| `currency`             | Да               | Код валюты ISO 4217 (3 заглавные буквы, например `RUB`, `USD`)    |
| `timestamp`            | Да               | Время операции ISO 8601 (не более 5 минут в будущее)              |
...
Примечание: для роли USER поле `userId` можно не передавать. Если передать, оно будет проигнорировано, фактический
`userId` берется из JWT (`sub`).
...
- USER может создавать транзакции только для себя: фактический `userId` всегда берется из JWT (`sub`), поле `userId` в
  теле запроса игнорируется (даже если передано).
...
- `from` — начало периода (ISO 8601, включительно).
- `to` — конец периода (ISO 8601, **исключительно**).
...
- `from`, `to` — период (ISO 8601, `from` включительно, `to` исключительно).
- `groupBy` — группировка: `hour`, `day` или `week` (по умолчанию `day`).
- `timezone` — часовой пояс (по умолчанию `UTC`).
- `channel` — опциональный фильтр по каналу (`WEB`, `MOBILE`, `POS`, `OTHER`).

> **Ограничения по groupBy:** `hour` — макс. 7 дней, `day` — макс. 90 дней, `week` — макс. 1 год. По умолчанию:последние
> 7 дней.
...
- `txCount` — количество транзакций.
- `gmv` — сумма транзакций (опционально).
...
Полные правила DSL вынесены в [dsl.md](./dsl.md) (уровни поддержки, грамматика, проверка, лимиты, «не падать из‑за
